...
function Work() {
  ...
  const [notesArr, setNotesArr] = useState(localNotes ? localNotes.filter(note => note.type === "work") : []);
  ...
  const handleSave = async () => {
    ...
    if (updatedId.current) {
      ...
      data.id = nanoid(10);
      data.type = "work"; // Set the type to "work"
    }
    if (data.blocks.length) {
      setNotesArr((prev) => [data, ...prev]);
      const allNotes = JSON.parse(localStorage.getItem("notes")) || [];
      localStorage.setItem("notes", JSON.stringify([data, ...allNotes.filter(note => note.type !== "work"), ...notesArr]));
    }
  };
